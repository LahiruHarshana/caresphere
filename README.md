...
**Project Title:** CareSphere - Trusted Caregiving Marketplace Platform

**Student Name:** Lahiru Harshana Jayasinghe

**Student ID:** 25021991

**Programme Context:** Computing final project / product-based software artefact

**Project Type:** Full-stack web application, research-informed technical report, and working prototype

**Report Target:** Approximately 15,000 words

**Submitted Artefact:** `caresphere` monorepo containing a Next.js frontend, NestJS backend API, Prisma database schema, Docker configuration, tests, implementation guides, and this project README report.

This README has been structured as a product-based project report rather than only a short installation guide. The report follows the academic layout guidance supplied with the project materials: preamble information, a main body containing introduction, literature review, methodology, investigation and analysis, design, implementation, product evaluation, critical project evaluation, and conclusion, followed by references, appendices, and a glossary. The project brief emphasises independent research, practical problem solving, artefact development, evidence-based analysis, and coherent written reporting. CareSphere is therefore documented as both a software product and a computing project, explaining not only what was built but also why key architectural, security, database, interface, and deployment decisions were made.

The implementation evidence checked for this version includes the root workspace configuration, `apps/api` NestJS modules, Prisma schema and migrations, authentication and role guards, booking workflow, matching service, real-time Socket.IO gateways, payments and invoices, vault encryption, reviews, notifications, uploads, admin tools, `apps/web` Next.js route groups, shared UI components, Docker files, end-to-end tests, and the supporting guide documents. Where the codebase still has prototype limitations, the report identifies them honestly in the evaluation and critical evaluation sections so that the document reads as a realistic final-year computing submission instead of a purely promotional product brochure.
...
The authentication module provides identity management with JWT access tokens, refresh tokens, email verification tokens, password reset tokens, and role-aware login responses. In the current NestJS application the controllers are mounted without a global `/api/v1` prefix, so the local route paths are `/auth/register`, `/auth/login`, `/auth/verify-email`, `/auth/resend-verification`, `/auth/forgot-password`, `/auth/reset-password`, `/auth/refresh`, `/auth/me`, and `/auth/logout`. A production gateway can still add `/api/v1` at the reverse proxy level, but the source code itself currently exposes the controller paths directly. Registration creates the base `User`, shared `Profile`, and role-specific `CustomerProfile` or `CaregiverProfile` records. Login validates Argon2 password hashes and returns user display fields plus token pairs. The `me` endpoint returns a normalized profile payload and blocks banned accounts. Logout is intentionally stateless and returns a success message because JWT invalidation is not stored server-side in this prototype.

### User Management Endpoints

The user management module supports customer dashboard aggregation, shared profile retrieval, customer profile retrieval, profile updates, customer preference updates, avatar upload, and public user detail retrieval. The implemented routes are `/users/dashboard`, `/users/profile`, `/users/customer-profile`, `/users/profile` by POST for updates, `/users/customer-profile` by POST for updates, `/users/avatar`, and `/users/:id`. Account deletion is not implemented in the current codebase and would require an explicit data retention policy before production launch.

### Caregiver Management Endpoints

The caregiver management module provides discovery, dashboard, profile management, verification document upload, avatar upload, cover upload, and profile detail capabilities. The implemented routes include `/caregivers/browse`, `/caregivers/dashboard`, `/caregivers/profile`, `/caregivers/profile` by POST for updates, `/caregivers/verify`, `/caregivers/upload-avatar`, `/caregivers/upload-cover`, and `/caregivers/:id`. The browse endpoint supports practical marketplace filtering, while the verification workflow stores submitted evidence for administrator review. The public caregiver detail route returns the caregiver profile, shared profile fields, and relevant review information for customer decision making.

### Booking Management Endpoints

The booking management module provides scheduling, conflict prevention, cost calculation, booking history, detail retrieval, status updates, and cancellation. The implemented routes are `/bookings` by POST, `/bookings` by GET, `/bookings/:id`, `/bookings/:id/status`, and `/bookings/:id/cancel`. Booking creation checks that the caregiver exists, is available, and is not already booked during the requested time range. Total cost is calculated from duration and the caregiver's hourly rate. Status updates follow the implemented finite state machine: `PENDING` can become `CONFIRMED` or `CANCELLED`, `CONFIRMED` can become `IN_PROGRESS` or `CANCELLED`, `IN_PROGRESS` can become `COMPLETED`, and terminal states cannot move further. Notification records are sent to the other party when important status changes occur.

### Matching Endpoints

The matching module provides intelligent caregiver discovery through `/matching/caregivers`. It retrieves available approved caregivers and ranks them with a weighted scoring model. Skills contribute up to 40 points, distance contributes up to 30 points, ratings contribute up to 20 points, and languages contribute up to 10 points. The service uses the Haversine formula for distance calculation when customer coordinates and caregiver coordinates are available. It filters by maximum distance when requested, removes password hash data from the response, sorts by total score, and returns the top ten matches.

### Review and Rating Endpoints

The review module provides booking-linked feedback while preventing reviews for incomplete bookings and preventing duplicate reviews by the same author for the same booking. The implemented routes are `/reviews` by POST, `/reviews/user/:userId`, and `/reviews/booking/:bookingId`. Review creation determines the target automatically: customers review caregivers, and caregivers review customers. User review listing supports page and limit query parameters and returns the review list, total count, current page, average rating, and total review count. Review editing, deletion, helpfulness voting, and reporting are not implemented yet, so those would be future moderation improvements rather than current product claims.

### Payment Endpoints

The payment module provides Stripe payment intent creation, webhook handling, refund placeholder behavior, and payment history retrieval. The implemented routes are `/payments/create-intent`, `/payments/refund`, `/payments/webhook`, and `/payments/history`. Payment intent creation verifies that the current customer owns the booking and that a total cost exists, then creates a Stripe PaymentIntent in USD cents. The webhook handler verifies the Stripe signature and processes `payment_intent.succeeded` by confirming the booking and marking an existing invoice as paid when one exists. Payment failure and cancellation event handling are not yet expanded. Refund support currently validates the invoice and returns a success response, but full Stripe refund execution and a `REFUNDED` invoice status would need schema and service changes.

### Vault Endpoints

The vault module provides encrypted family information storage through `/vault`, `/vault/:id`, `/vault/:id/grant`, and `/vault/:id/revoke`. Entries are encrypted with AES-256-GCM before persistence, with the IV stored separately and the authentication tag stored alongside ciphertext. Customers can list and retrieve their own entries. Caregivers can retrieve an entry only if their user ID has been granted access and an active `IN_PROGRESS` booking exists with the customer. Grant and revoke endpoints update the `allowedCaregiverIds` array. A delete endpoint and detailed vault audit log are not implemented yet; these are sensible future additions because sensitive care data should have explicit lifecycle controls.

### Invoice Endpoints

The invoice module provides PDF generation and download through `/invoices/generate` and `/invoices/:id/download`. Invoice generation is available to customers and administrators after role guard validation. The service loads the booking, checks authorization, calculates total cost if needed, creates an invoice record when one does not already exist, renders an invoice HTML template through Puppeteer, writes a PDF into the API runtime `tmp` directory, and stores the generated file path on the invoice record. The download endpoint streams the PDF with `application/pdf` and attachment headers. Cloudinary storage is not currently used for generated invoices.

### Admin Endpoints

The admin module provides platform management capabilities through `/admin/users`, `/admin/users/:id/ban`, `/admin/caregivers/pending`, `/admin/caregivers/:userId/verify`, `/admin/logs`, `/admin/analytics`, `/admin/bookings`, and `/admin/bookings/:id/cancel`. These routes are protected by JWT authentication, `RolesGuard`, and the `ADMIN` role. User listing supports search and role filtering. Ban toggling writes audit logs. Caregiver verification updates verification status, sends an in-app notification, and records an audit action. Analytics returns totals for users, caregivers, customers, bookings, pending verifications, completed-booking revenue, monthly revenue, monthly registrations, booking status grouping, and recent logs. Booking administration supports status filtering, pagination, search, and admin cancellation.
...
The Profile model defines shared user information across all role types. It stores first name, last name, avatar URL, biography, phone, address, latitude, longitude, languages, and optional date of birth. Latitude and longitude are indexed together so the matching service can efficiently work with location-aware caregiver discovery. Languages use PostgreSQL array storage through Prisma, allowing the matching algorithm to compare requested languages with caregiver profile languages.

### CustomerProfile Model

The CustomerProfile model defines care-seeker preferences and requirements. It stores care type, care frequency, special requirements, preferred schedule, emergency contact, emergency phone, and medical notes as optional string fields. This structure allows the registration and profile editing flows to preserve customer-specific care context without forcing every user to provide sensitive information during account creation.

### CaregiverProfile Model

The CaregiverProfile model defines care provider qualifications and verification status. It stores hourly rate, years of experience, certifications, specialties, verification status, background check URL, cover photo URL, availability, background-check agreement, and biography. Specialty and certification fields use PostgreSQL arrays through Prisma. Verification status is constrained to `PENDING`, `APPROVED`, and `REJECTED`, and indexed so the admin dashboard and matching service can quickly separate approved caregivers from pending profiles.

### Booking Model

The Booking model defines service engagement records connecting customers and caregivers. It stores customer ID, caregiver ID, service type, scheduled start, end time, status, total cost, notes, creation time, and update time. Status is constrained to `PENDING`, `CONFIRMED`, `IN_PROGRESS`, `COMPLETED`, and `CANCELLED`, matching the service-level state machine. The model is indexed by customer, caregiver, status, and scheduled time to support dashboards, history pages, admin filtering, and conflict checking.

### Review Model

The Review model defines post-service feedback with rating and optional comment storage. Each review is linked to a booking, an author, and a target user. The service layer enforces the most important business rules: reviews can only be created for completed bookings, the reviewer must be part of the booking, and a user cannot submit more than one review for the same booking. Author, target, and booking indexes support review lookups for profile pages and booking detail pages.
...
The Invoice model defines payment documentation with Stripe integration. It stores booking ID, amount, currency, status, generated PDF path, Stripe payment intent ID, issue time, and creation time. Status is constrained to `PENDING`, `PAID`, and `OVERDUE` in the current Prisma schema. Generated invoice PDFs are stored on local disk in the prototype and streamed by the invoice download endpoint.

### Earning Model

The Earning model defines caregiver revenue tracking for completed work. It has a unique booking relationship, amount, status, creation time, and update time. Status is constrained to `PENDING` and `PAID` in the current schema. The model is indexed by booking and status so caregiver earnings views and future payout workflows can retrieve records efficiently.
...
CareSphere was developed and documented by Lahiru Harshana Jayasinghe as a Computing final project artefact. I would like to acknowledge the academic guidance provided through the project module, especially the emphasis on independent research, practical software development, clear technical reporting, and honest critical evaluation. The project brief helped shape the report structure by encouraging a product-based dissertation format that moves from problem investigation through design, implementation, evaluation, and conclusion.

Special acknowledgment goes to the open-source community for the frameworks and libraries that made the project possible, including Next.js and React for the frontend application, NestJS for the backend architecture, Prisma for type-safe database access, PostgreSQL for relational data storage, Socket.IO for real-time communication, Stripe for payment integration, TailwindCSS for interface styling, Jest and Playwright for testing, and Docker for deployment consistency. These tools allowed the project to focus on the caregiving marketplace problem while still applying modern professional engineering practices.

I also acknowledge the support of family, friends, lecturers, and peers who encouraged the development of this project. Their feedback helped keep the platform focused on a meaningful real-world problem: making it easier for families to find trusted caregivers and giving caregivers a clearer, safer way to manage their work.
