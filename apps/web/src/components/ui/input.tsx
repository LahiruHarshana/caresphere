...
const Input = forwardRef<HTMLInputElement, InputRef>(
  ({ className, dark, ...props }, ref) => {
    let classes = dark
      ? "dark-input flex h-11 w-full border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder:text-white/30 transition-all duration-300 focus-visible:outline-none focus-visible:border-[#0d9488] focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50 rounded-sm"
